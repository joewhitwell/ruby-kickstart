...
def problem_14 (*params)
    problem = params.pop[:problem] if params.last.is_as? Hash
    problem ||= :count_clumps
...
numbers.each do |number|
    clumps += 1 if (previous == number) && (previous != two_before) #count clump only once
    two_before = previous # updates as progresses
    previous = number 
end
...
def same_ends (n,*params)
    params [0, n] == params[-n,n] #take the range at the start and the range at the end
end
